...
class Home extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.home', [
            ...
            'latestEpisodes' => ($settings?->show_episodes ?? true) ? Episode::with(['characters.job', 'characters.socialLinks'])->withAvg('ratings', 'rating')->where('category', 'episode')->where('visible', true)->latest()->take(5)->get() : collect(),
            'latestShorts' => ($settings?->show_shorts ?? true) ? Episode::with(['characters.job', 'characters.socialLinks'])->withAvg('ratings', 'rating')->where('category', 'short')->where('visible', true)->latest()->take(5)->get() : collect(),
            'latestMinis' => ($settings?->show_minis ?? true) ? Episode::with(['characters.job', 'characters.socialLinks'])->withAvg('ratings', 'rating')->where('category', 'mini')->where('visible', true)->latest()->take(5)->get() : collect(),
