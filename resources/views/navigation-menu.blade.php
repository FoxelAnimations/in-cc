<nav x-data="{ open: false }" class="relative z-40 bg-black text-white">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:space-x-10">
                ...
                @if(\App\Models\SiteSetting::first()?->show_map || auth()->user()?->is_admin)
    ...
    >
        ...
        <div class="flex-1 flex flex-col items-center justify-center space-y-6">
            ...
            @if(\App\Models\SiteSetting::first()?->show_map || auth()->user()?->is_admin)
                <a href="{{ route('map') }}" @click="open = false" class="{{ request()->routeIs('map') ? 'text-[#E7FF57]' : 'text-white' }} text-2xl tracking-widest hover:text-[#E7FF57] transition">DE KAART</a>
            @endif
            @auth
                @can('access-admin')
